...
export function NoteComposer({ onCreate, defaultPage }: NoteComposerProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-sm max-w-none text-slate-800 dark:prose-invert dark:text-slate-100",
      },
    },
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 rounded-full bg-slate-100/70 px-3 py-1 text-xs font-medium text-slate-600 dark:bg-slate-800/60 dark:text-slate-200">
          Trình soạn thảo ghi chú phong phú
        </div>
      </div>
