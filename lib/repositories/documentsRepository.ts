...
function normaliseBinaryData(data: any | Buffer | Binary): Buffer {
...
export async function listStoredDocuments(userId: string): Promise<StoredDocumentMetadata[]> {
  const db = await getMongoDatabase()
  const collection = db.collection<DocumentRecord>(COLLECTIONS.documents)
  const documents = await collection
    .find<StoredDocumentMetadata & { _id: ObjectId }>(
      { userId },
      {
        projection: { data: 0 },
        sort: { createdAt: -1 },
